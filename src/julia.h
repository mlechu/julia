...
typedef struct _jl_sourcebytetable_header_t {
    /* minimum byte */
    int32_t byte_offset;
    /* minimum line, where line_starts[0] is the byte position of this line's
     * first character */
    int32_t line_offset;
    /* number of (byte, len) spans */
    int32_t nlocs;
    uint8_t byte_encl;
    uint8_t span_encl;
    // in compressed string, followed by:
    // spans: (byte_encl+span_encl)*nlocs bytes
