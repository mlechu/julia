        ...
        ((eq? (car e) 'lambda)
         (let ((bound (table)))
           ...
           (for-each (lambda (v) (if (not (has? bound v)) (put! tab v #t)))
                     (table.keys (free-vars (lam:body e)))))
         tab)
        (else
...
(define (analyze-vars-lambda e env captvars sp new-sp methsig tab)
  (trace-let* ((args (lam:args e))
               (locl (lam:vinfo e))
         (allv (list->tset (nconc (map arg-name args) locl)))
         (fv   (trace-let* ((fv (tset.subtract! (free-vars (lam:body e)) allv))
                      ;; add variables referenced in declared types for free vars
                      (dv (map (lambda (v)
                                 (let ((vi (get tab v #f)))
                                   (if vi (table.keys (free-vars (vinfo:type vi))) '())))
                               (table.keys fv)))
                      (dv (list->tset (apply nconc dv))))
                 (tset.union! fv dv)))
         (sig-fv (if methsig (free-vars methsig) (table)))
         ;; make var-info records for vars introduced by this lambda
         (vi   (nconc
                (map (lambda (decl) (make-var-info (decl-var decl))) args)
                (map make-var-info locl)))
         (capt-sp (filter (lambda (v) (or (and (tset.has? fv v) (not (memq v new-sp)))
                                          (tset.has? sig-fv v)))
                          sp))
         ;; captured vars: vars from the environment that occur
         ;; in our set of free variables (fv).
         (cv    (append (filter (lambda (v) (and (tset.has? fv (vinfo:name v))
                                                 (not (memq (vinfo:name v) new-sp))))
                                env)
                        (map make-var-info capt-sp)))
         (new-env (append vi
                          ;; new environment: add our vars
                          (filter (lambda (v) (not (tset.has? allv (vinfo:name v))))
                                  env))))
    ...
    ((symbol? var)
     (let* ((vi (get locals var #f))
       ...
       (if (and (not closed) (not capt) (equal? vt '(core Any)))
           (if (or (local-in? var lam locals) (underscore-symbol? var))
               `(= ,var ,rhs0)
...
(define (definitely-initialized-vars stmts vi)
  (trace-let* ((vars (table))
        (di   (table))
        (vi   (vinfo-to-table vi)))
