...
"""
Key to use when transforming names into bindings.  Analogous to a mangled name
in macroexpand.scm.
"""
struct NameKey
    name::String
...
Base.isless(a::NameKey, b::NameKey) = (a.name, a.layer) < (b.name, b.layer)
Base.isless(a::IdTag, b::NameKey) = true
Base.isless(a::NameKey, b::IdTag) = false
...
const VarKey = Union{NameKey, IdTag}

VarKey(ex::SyntaxTree) = kind(ex) === K"BindingId" ? ex.var_id : NameKey(ex)

struct ScopeInfo
    id::ScopeId        # index into ctx.scopes
    parent_id::ScopeId # 0 if top-level thunk
    lambda_id::ScopeId # Own ID if lambda, else some parent ID
    node_id::NodeId    # Tree introducing this scope
    ...
    is_permeable::Bool
    assignments::Dict{VarKey, NodeId}
    # Map from variable names to binding IDs in this scope.  Does not include
    # variables in-scope due to being inherited from the parent scope.
    # * Locals, explicitly-declared or implicit from assignment
    # * Explicit globals
    # * Arguments and static parameters
    # * The top-level scope will also contain all implicit globals for
    #   resolution to fall back to.
    vars::Dict{NameKey,IdTag}
    # Variable usage info (only if this is a lambda scope)
...
function ScopeInfo(ctx, parent_id, node_id, is_lambda, is_permeable)
    ...
    s = ScopeInfo(
        ...
        Dict{VarKey, NodeId}(), Dict{NameKey,IdTag}(),
...
struct ScopeResolutionContext{GraphType} <: AbstractLoweringContext
    ...
    # Globals that may be assigned to without the `global` keyword in soft
    # scopes due to being assigned to at top level, or passing the
    # defined-and-owned-global check.
...
enclosing_lambda(ctx, ::Nothing) = nothing
parent(ctx, scope::ScopeInfo) = is_top_scope(scope) ? nothing :
    ctx.scopes[scope.parent_id]

lam_scope(ctx, lb::LambdaBindings) = ctx.scopes[lb.scope_id]
enclosing_lambda(ctx, lb::LambdaBindings) = ctx.scopes[lb.scope_id]
parent(ctx, lb::LambdaBindings) = parent(ctx, lam_scope(ctx, lb))
...
function maybe_declare_in_scope!(ctx, scope::ScopeInfo, ex, new_k::Symbol)
    if kind(ex) === K"BindingId"
        ...
        ensure_lambda_local!(ctx, scope, get_binding(ctx, bid), false)
        return bid
    ...
    #     declare_in_scope!(ctx, scope, ex, :global)
        ...
        $(_var_str(new_k)) name `$(NameKey(ex).name)` conflicts with a(n) \
        $(_var_str(old_k)) from the same scope
        """))
    end
end
...
function declare_in_scope!(ctx, scope::ScopeInfo, ex, bk::Symbol; kws...)
    ...
    ensure_lambda_local!(ctx, scope, b, false)
    return b.id
end

# If `b` is local and not yet recorded in the lambda bindings, mark it as
# `capt`.  Also (if `capt==true`) mark captured in any parent lambdas.
function ensure_lambda_local!(ctx, scope::Union{ScopeInfo, LambdaBindings}, b, capt)
    if b.kind === :global || b.is_ssa
        return
    end
    lam_capts = enclosing_lambda(ctx, scope).locals_capt
    if !haskey(lam_capts, b.id)
        lam_capts[b.id] = capt
        lam2 = enclosing_lambda(ctx, parent(ctx, scope))
        if capt && !b.is_captured
            update_binding!(ctx, b.id; is_captured=true)
        end
        if capt && !isnothing(lam2) && !haskey(lam2.locals_capt, b.id)
            ensure_lambda_local!(ctx, lam2, b, capt)
        end
    end
end
...
function resolve_name(ctx, ex; exclude_toplevel_globals=false)
    for sid in reverse(ctx.scope_stack)
        ...
        if !exclude_toplevel_globals || b.kind !== :global || sid !== top_scope(ctx).id
            return b
        end
    end
end
...
function _find_scope_decls!(ctx, scope, ex)
    ...
    elseif (k in KSet"= constdecl assign_or_constdecl_if_global" &&
        kind(ex[1]) in KSet"Identifier BindingId")
        get!(scope.assignments, VarKey(ex[1]), ex[1]._id)
        if k != K"constdecl" || numchildren(ex) == 2
            _find_scope_decls!(ctx, scope, ex[2])
        end
    elseif k == K"function_decl"
        if kind(ex[1]) in KSet"Identifier BindingId"
            get!(scope.assignments, VarKey(ex[1]), ex[1]._id)
...
function enter_scope!(ctx, ex)
    ...
    for (vk, node_id) in sort!(collect(scope.assignments))
        ...
        if vk isa IdTag
            maybe_declare_in_scope!(ctx, scope, ex, get_binding(ctx, vk).kind)
            continue
        end
        ...
        elseif b.kind === :local || b.kind === :argument
            ...
            ensure_lambda_local!(ctx, scope, b, true)
        end
    end
...
function _resolve_scopes(ctx, ex::SyntaxTree, @nospecialize(scope))
    ...
    if k == K"Identifier"
        ...
        ensure_lambda_local!(ctx, scope, b, true)
        makeleaf(ctx, ex, K"BindingId"; var_id=b.id)
    elseif k === K"BindingId"
        ensure_lambda_local!(ctx, scope, get_binding(ctx, ex), true)
        ex
...
function analyze_variables!(ctx, ex)
    ...
    if k == K"BindingId"
        ...
        !b.is_read && update_binding!(ctx, b.id, is_read=true)
        # The type of typed locals is invisible in the previous pass,
        # but is filled in here.
        ensure_lambda_local!(ctx, ctx.lambda_bindings, b, true)
    ...
    elseif k == K"="
        ...
        if kind(lhs) != K"Placeholder"
            lhs_b = get_binding(ctx, lhs)
            update_binding!(ctx, lhs_b.id, add_assigned=1)
            ensure_lambda_local!(ctx, ctx.lambda_bindings, lhs_b, true)
            if !isnothing(lhs_b.type)
                # Assignments introduce a variable's type later during closure
                # conversion, but we must model that explicitly here.
                analyze_variables!(ctx, lhs_b.type)
            end
        end
    ...
    elseif k == K"function_decl"
        ...
        update_binding!(ctx, b.id, add_assigned=1)
    elseif k == K"function_type"
        if kind(ex[1]) != K"BindingId" || get_binding(ctx, ex[1]).kind !== :local
            analyze_variables!(ctx, ex[1])
        end
    elseif k == K"constdecl"
        update_binding!(ctx, ex[1].var_id; is_const=true)
    elseif k == K"call"
        name = ex[1]
        if kind(name) == K"BindingId"
            b = get_binding(ctx, name.var_id)
            !b.is_called && update_binding!(ctx, b.id, is_called=true)
        end
