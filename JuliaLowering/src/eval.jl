...
function lower_init(ex::SyntaxTree{T}, init_world::UInt;
                    expr_compat_mode::Bool=false) where {T}
    LoweringIterator{T}(init_world, expr_compat_mode, [(ex, false, 0)])
end

function lower_step(iter, mod, world=iter.init_world)
    ...
    if child_idx > 0
        ...
        elseif is_module_body
            ...
            return lower_step(iter, mod, Base.get_world_counter())
        end
    else
        ex = top_ex
    end
...
@fzone "JL: eval" function eval(mod::Module, ex::SyntaxTree;
                                ...
                                opts...)
    iter = lower_init(ex, macro_world; opts...)
    _eval(mod, iter)
end
...
function _eval(mod, iter)
    ...
    result = nothing
    while true
        thunk = lower_step(iter, modules[end],
                           Base.get_world_counter())::Core.SimpleVector
        type = thunk[1]::Symbol
        if type == :done
            break
