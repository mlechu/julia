...
function _encoded_len(max::Int32)
    @assert max >= 0 "$max"
    return Int32(max == 0 ? 0 : max < 256 ? 1 : max < 65536 ? 2 : 4)
end

function compress_sbt(sbt::SourceByteTable)
    min_byte = sbt.line_starts[1]
    max_byte = sbt.spans[end][end]
    max_span = Int32(0);
    for (b1,b2) in sbt.spans
        max_span = max(max_span, (b2+Int32(1))-b1)
    end

    max_byte_rel = Int32(min_byte > max_byte ? 0 : (max_byte - min_byte))
    nlocs::Int32 = length(sbt.spans)
    final_len = 14 + # header
        (_encoded_len(max_byte_rel) + _encoded_len(max_span)) * nlocs +
        (_encoded_len(max_byte_rel) * length(sbt.line_starts))

    io = IOBuffer(;sizehint=final_len);
    _push32(io, min_byte, 4)
    _push32(io, sbt.line_offset, 4)
    _push32(io, nlocs, 4)
    _push32(io, _encoded_len(max_byte_rel), 1)
    _push32(io, _encoded_len(max_span), 1)
    for (b1, b2) in sbt.spans
        _push32(io, b1 - min_byte, _encoded_len(max_byte_rel))
        _push32(io, b2 - b1 + Int32(1), _encoded_len(max_span))
    end
    @assert issorted(sbt.line_starts)
    @assert allunique(sbt.line_starts)
    for n in sbt.line_starts
        _push32(io, n - min_byte, _encoded_len(max_byte_rel))
    end
...
function macro_sourceref(st::SyntaxTree)

    st.source isa NodeId || return st
    msrc = get(st, :macro_source, nothing)
    isnothing(msrc) ? sourceref(st) :
        macro_sourceref(SyntaxTree(st._graph, msrc))
end
...
function collect_locs!(node_sources, codeinfos, top_sf, st, toplevel)
    if kind(st) === K"code_info"
        push!(codeinfos, st)
        node_sources[st._id] = _di_pos(st) # should probably handle inconsistency here
